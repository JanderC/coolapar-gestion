...
/** Suma (o resta) días a una fecha en texto yyyy-mm-dd, sin líos de zona horaria. */
...
<body>
  ...
  // ---------- Consultar semana: todos los productores del rango ----------
  // Esta vista va al revés que el resto de la pantalla: en lugar de partir
  // de un productor y ver su semana, parte de la semana y trae a todos los
  // que dejaron leche en esos días, cada uno con su propio precio.
  ...
  const detalleError = (err) => {
    if (err?.response) {
      ...
      return data?.message || `El servidor respondió ${status}.`;
    }
  ...
  const consultarSemana = async (desde = semanaDesde, hasta = semanaHasta) => {
    if (!desde || !hasta) {
      setErrorResumen('Indique la fecha de inicio y la de cierre.');
      return;
    }
    if (desde > hasta) {
      setErrorResumen('La fecha de inicio debe ser anterior a la de cierre.');
      return;
    }
    if (typeof registroApi.resumenSemana !== 'function') {
      setErrorResumen(
        "Falta agregar resumenSemana() en src/api/registroLeche.api.js: export const resumenSemana = (params) => api.get('/registros-leche/resumen-semana', { params });"
      );
      return;
    }
    ...
    setErrorResumen('');
    try {
      const datos = desempacar(await registroApi.resumenSemana({ fecha_inicio: desde, fecha_fin: hasta }));
      setResumen(datos);
      setRangoResumen({ inicio: desde, fin: hasta });
      setSeleccionResumen([]);
    } catch (err) {
      console.error('resumen-semana:', err);
      setResumen(null);
      setErrorResumen(`No se pudo cargar la semana. ${detalleError(err)}`);
    } finally {
  ...
  const filasResumen = useMemo(() => {
    const lista = resumen?.productores || [];
    const q = plano(busquedaResumen);
    return q ? lista.filter((p) => plano(p.nombre).includes(q)) : lista;
  }, [resumen, busquedaResumen]);

  const todosResumenMarcados =
    filasResumen.length > 0 && filasResumen.every((p) => seleccionResumen.includes(String(p.productor_id)));
  ...
  const alternarTodosResumen = () => {
    const visibles = filasResumen.map((p) => String(p.productor_id));
    setSeleccionResumen((prev) =>
      todosResumenMarcados ? prev.filter((x) => !visibles.includes(x)) : [...new Set([...prev, ...visibles])]
    );
  };

  /** Totales de lo marcado, separados por moneda (BS y USD no se suman). */
  const totalesSeleccionResumen = useMemo(() => {
    const mapa = new Map();
    (resumen?.productores || [])
      .filter((p) => seleccionResumen.includes(String(p.productor_id)))
      .forEach((p) => {
        if (!mapa.has(p.moneda)) mapa.set(p.moneda, { moneda: p.moneda, productores: 0, total_litros: 0, total_pagar: 0 });
        const t = mapa.get(p.moneda);
        t.productores += 1;
        t.total_litros += aNumero(p.total_litros, 0);
        t.total_pagar += aNumero(p.total_pagar, 0);
      });
    return [...mapa.values()].sort((a, b) => a.moneda.localeCompare(b.moneda));
  }, [resumen, seleccionResumen]);

  /** Hoja final con el total de todos los productores impresos. */
  const construirBloqueTotales = (totales) => {
      ...
      .map(
        (t) => `
          <tr>
            ...
            <td class="num">${t.total_litros.toFixed(2)} L</td>
            <td class="num">${formatearMontoEnMoneda(t.total_pagar, t.moneda)}</td>
          </tr>`
      )
      .join('');
    return `
  <div class="bloque">
    ...
    <table>
      <thead>
        <tr><th>Moneda</th><th class="num">Productores</th><th class="num">Litros</th><th class="num">Total a pagar</th></tr>
      </thead>
      <tbody>${filas}</tbody>
    </table>
  </div>`;
  ...
  const imprimirResumen = async () => {
    ...
    setErrorResumen('');
    try {
      // Se respeta el orden de la tabla, no el orden en que se marcaron.
      const ids = (resumen?.productores || [])
        .map((p) => String(p.productor_id))
        .filter((id) => seleccionResumen.includes(id));

      const hojas = await Promise.all(
        ids.map((id) =>
          registroApi
            .obtenerHoja({ productor_id: id, fecha_inicio: rangoResumen.inicio, fecha_fin: rangoResumen.fin })
            .then(desempacar)
        )
      );

      const bloques = hojas.map((h) => construirBloqueProductor(h));
      if (ids.length > 1) bloques.push(construirBloqueTotales(totalesSeleccionResumen));
      ...
      console.error('impresion resumen-semana:', err);
      setErrorResumen(`No se pudo preparar la impresión. ${detalleError(err)}`);
    } finally {
        ...
        <Card className="mb-4 border-success">
          ...
          {cargandoResumen ? (
            ...
            <>
              <div className="px-3 pb-2 d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                  ...
                  <div className="fs-5">
                    ...
                    <strong>{resumen.totales.total_litros}</strong> litros recibidos
              ...
              <div className="px-3 pb-2 d-flex flex-wrap justify-content-between align-items-center gap-2">
                <Form.Control
                  type="search"
                  size="sm"
                ...
                <div className="d-flex align-items-center gap-2">
                  {seleccionResumen.length > 0 &&
                    totalesSeleccionResumen.map((t) => (
                      <span key={t.moneda} className="text-muted small">
              ...
              <Table hover responsive className="mb-0 align-middle small">
                <thead>
                  <tr>
                    ...
                    <th style={{ minWidth: 170 }}>Productor</th>
                ...
                <tbody>
                  {filasResumen.map((p) => {
                    const marcado = seleccionResumen.includes(String(p.productor_id));
                    return (
                      <tr
                        key={p.productor_id}
                        className={marcado ? 'table-success' : undefined}
                        style={{ cursor: 'pointer' }}
                        onClick={() => alternarResumen(p.productor_id)}
                      >
                        <td onClick={(e) => e.stopPropagation()}>
                          <Form.Check
                            type="checkbox"
                            checked={marcado}
                            onChange={() => alternarResumen(p.productor_id)}
                        ...
                        {p.dias.map((d) => (
                          <td key={d.fecha} className="text-end">
                            ...
                            {(d.litros_acidos > 0 || d.litros_bajo_grasa > 0) && (
                              <div className="text-muted" style={{ fontSize: '.7rem' }}>
                                {d.litros_acidos > 0 && `${d.litros_acidos} ác.`}
                                {d.litros_acidos > 0 && d.litros_bajo_grasa > 0 && ' · '}
                                {d.litros_bajo_grasa > 0 && `${d.litros_bajo_grasa} b.g.`}
                              </div>
                            )}
                          </td>
                        ))}
                        <td className="text-end fw-semibold">
                          {p.total_litros}
                          {(p.total_litros_acidos > 0 || p.total_litros_bajo_grasa > 0) && (
                            <div className="text-muted fw-normal" style={{ fontSize: '.7rem' }}>
                              {p.total_litros_acidos > 0 && `${p.total_litros_acidos} ác.`}
                              {p.total_litros_acidos > 0 && p.total_litros_bajo_grasa > 0 && ' · '}
                              {p.total_litros_bajo_grasa > 0 && `${p.total_litros_bajo_grasa} b.g.`}
                            </div>
                          )}
                        </td>
                        ...
                        <td>
                          {p.estado_pago === 'pagado' ? (
                            ...
                            <Badge bg="dark">Cerrada</Badge>
                          ) : p.guardado ? (
                            <Badge bg="info">Guardada</Badge>
                          ) : (
                            <Badge bg="warning" text="dark">
                              Sin guardar
                            </Badge>
                  ...
                  {filasResumen.length === 0 && (
                    <tr>
                      <td colSpan={(resumen.rango.columnas || []).length + 6} className="text-center text-muted py-3">
                        Ningún productor de esta semana coincide con «{busquedaResumen}».
                      </td>
                    </tr>
                  )}
                </tbody>
                <tfoot className="table-light">
                  <tr>
                    <th />
                    <th>Litros recibidos por día</th>
